...
BEGIN
	SELECT u.ID_USUARIO, dp.NOMBRE, dp.APELLIDO_1, dp.APELLIDO_2, dp.TELEFONO, 
		   ...
		   u.FECHA_NACIMIENTO, u.CORREO, u.TIPO_USUARIO, u.CONTRASENA, u.SUBSCRIPTION
	FROM USUARIOS u
...
CREATE PROCEDURE ObtenerHistorial20(
	IN p_id_usuario INT
)
BEGIN
	SELECT dl.*, lv.ID_LIBRO_VIRTUAL
	...
	JOIN LIBRO_VIRTUAL lv ON dl.ID_DATOS_LIBRO = lv.ID_DATOS_LIBRO
	WHERE vl.ID_USUARIO = p_id_usuario
	ORDER BY dl.ID_DATOS_LIBRO DESC
	LIMIT 20;
END //

CREATE PROCEDURE BuscarLibrosVirtuales(
    IN p_clave VARCHAR(50)
)
BEGIN
    SELECT dl.ID_DATOS_LIBRO, dl.TITULO, dl.EDITORIAL, dl.EDICION, lv.RESUMEN, lv.ID_LIBRO_VIRTUAL, dl.FECHA_PUBLICACION, dl.AUTORES, lv.archivo, lv.imagen, vlv.VALORACION
    FROM DATOS_LIBRO dl
    JOIN LIBRO_VIRTUAL lv ON dl.ID_DATOS_LIBRO = lv.ID_DATOS_LIBRO
    JOIN valoraciones_libros vlv ON lv.ID_DATOS_LIBRO = vlv.ID_DATOS_LIBRO
    WHERE dl.TITULO LIKE CONCAT('%', p_clave, '%') OR dl.AUTORES LIKE CONCAT('%', p_clave, '%');
END //

CREATE PROCEDURE getValoracionLibro(
	IN p_id_libro INT
)
BEGIN
	SELECT AVG(VALORACION) AS VALORACION_PROMEDIO
	FROM VALORACIONES_LIBROS
	WHERE ID_DATOS_LIBRO = p_id_libro;
END //
	

CREATE PROCEDURE ActualizarSubscription(
    IN p_id_usuario INT,
    IN p_nueva_suscripcion INT
)
BEGIN
    UPDATE USUARIOS
...
CREATE VIEW VistaMejoresLibros AS
SELECT dl.ID_DATOS_LIBRO, dl.TITULO, dl.EDITORIAL, dl.EDICION, dl.FECHA_PUBLICACION, AVG(vl.VALORACION) AS VALORACION_PROMEDIO
FROM DATOS_LIBRO dl
JOIN VALORACIONES_LIBROS vl ON dl.ID_DATOS_LIBRO = vl.ID_DATOS_LIBRO
GROUP BY dl.ID_DATOS_LIBRO, dl.TITULO, dl.EDITORIAL, dl.EDICION, dl.FECHA_PUBLICACION
ORDER BY VALORACION_PROMEDIO DESC
LIMIT 20;


-- DROP VIEW VistaMejoresLibrosConIDVirtual;
CREATE VIEW VistaMejoresLibrosConIDVirtual AS
SELECT dl.ID_DATOS_LIBRO, dl.TITULO, dl.EDITORIAL, dl.EDICION, dl.FECHA_PUBLICACION, AVG(vl.VALORACION) AS VALORACION_PROMEDIO, lv.ID_LIBRO_VIRTUAL, dl.AUTORES, lv.RESUMEN
FROM DATOS_LIBRO dl
JOIN VALORACIONES_LIBROS vl ON dl.ID_DATOS_LIBRO = vl.ID_DATOS_LIBRO
JOIN LIBRO_VIRTUAL lv ON dl.ID_DATOS_LIBRO = lv.ID_DATOS_LIBRO
GROUP BY dl.ID_DATOS_LIBRO, dl.TITULO, dl.EDITORIAL, dl.EDICION, dl.FECHA_PUBLICACION, lv.ID_LIBRO_VIRTUAL
ORDER BY VALORACION_PROMEDIO DESC
LIMIT 20;
...
UPDATE USUARIOS SET SUBSCRIPTION = 1 WHERE ID_USUARIO IN (1, 2, 3, 4);
